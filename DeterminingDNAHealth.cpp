...
int main()
{
    ...
    for (int s_itr = 0; s_itr < s; s_itr++) {
        ...
        for (int i=first; i<=last; i++){
            int index_find = d.find(genes[i]);
            while (index_find != string::npos){
                total_health += health[i];
                index_find = d.find(genes[i], index_find+1);
            } 
        }
        health_val.push_back(total_health);
    }
    
    int unhealthiest = *min_element(health_val.begin(), health_val.end());
    int healthiest = *max_element(health_val.begin(), health_val.end());
    
    cout<<unhealthiest<<" "<<healthiest;
